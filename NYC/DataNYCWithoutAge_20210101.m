...
%%%% Death
day = 12;
aux = data1(1+day:end,4)./data1(1:end-day,1);
aux = aux(inicio:fim);
...
%%%% Death in Hosp
day = 11;
aux = data1(1+day:end,4)./data1(1:end-day,3);
aux = aux(inicio:fim);
...
hold on
box on
title('Hospitalization Rate - NYC')
...
hold off
if SaveFig == 1
saveas(gcf,['HospRate_NYC',num2str(zz),'.fig']);
print('-dpng',['HospRate_NYC',num2str(zz)]);
end
...
hold on
box on
title('Death Rate - NYC')
plot(t_span(2+day:end),100*min(1,data1(1+day:end,4)./data1(1:end-day,1)),'b');
...
hold off
if SaveFig == 1
saveas(gcf,['DeathRate_NYC',num2str(zz),'.fig']);
print('-dpng',['DeathRate_NYC',num2str(zz)]);
end

figure
hold on
box on
title('Death Rate in Hospital - NYC')
plot(t_span(2+day:end),100*min(1,data1(1+day:end,4)./data1(1:end-day,3)),'b');
...
hold off
if SaveFig == 1
saveas(gcf,['DeathInHospRate_NYC',num2str(zz),'.fig']);
print('-dpng',['DeathInHospRate_NYC',num2str(zz)]);
end
...
% grid on
box on
title('Positive Tests - NYC')
plot(t_span(5:end),100*data2(:,2)./data2(:,1),'b');
...
hold off
if SaveFig == 1
saveas(gcf,'PosRate_NYC.fig');
print('-dpng','PosRate_NYC');
end
...
box on
title('Tests')
plot(t_span(5:end),data2(:,1),'r','LineWidth',1)
plot(t_span(5:end),data2(:,2),'--b','LineWidth',1)
...
hold off
if SaveFig == 1
saveas(gcf,'Tests_NYC.fig');
print('-dpng','Tests_NYC');
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Corrections
...
%%%%% By Death Rate
day = 12;
CorrTotalD = max(1,min(1,data1(1+day:end,4)./data1(1:end-day,1))/MeanDeath);
CorrTotalDCI90A = max(1,min(1,data1(1+day:end,4)./data1(1:end-day,1))/DeathCI90(1));
CorrTotalDCI90B = max(1,min(1,data1(1+day:end,4)./data1(1:end-day,1))/DeathCI90(2));
...
hold on
box on
title('Corrected Daily Infections')
...
hold off
if SaveFig == 1
saveas(gcf,'Infections_NYC.fig');
print('-dpng','Infections_NYC');
end
...
h1.Annotation.LegendInformation.IconDisplayStyle = 'off';
h2.Annotation.LegendInformation.IconDisplayStyle = 'off';
area([t_span(2),t_span(inicio)],[80000,80000],'FaceAlpha',0.3,'FaceColor',[249,208,160]/255,'linestyle',':')
area([t_span(inicio),t_span(fim)],[80000,80000],'FaceAlpha',0.3,'FaceColor',[133,193,233]/255,'linestyle',':')
area([t_span(fim),t_span(end)],[80000,80000],'FaceAlpha',0.3,'FaceColor',[193,253,199]/255,'linestyle',':')
legend('Reported','Corrected')
ylabel('Number of Individuals')
xlim([t_span(1),t_span(end-day)])
% ylim([0,7000])
...
hold off
if SaveFig == 1
saveas(gcf,['InfectionsD_NYC',num2str(zz)','.fig']);
print('-dpng',['InfectionsD_NYC',num2str(zz)']);
end
